...
@Composable
internal fun FavouriteRouteScreen(
    modifier: Modifier = Modifier,
    navigateToPlayer: (musicId: String) -> Unit = {},
    viewModel: FavouriteViewModel = hiltViewModel()
) {
    val favouriteSongs by viewModel.favouriteSongs.collectAsStateWithLifecycle()
...
internal fun FavouriteScreen(
    modifier: Modifier = Modifier,
    favouriteSongs: List<MusicEntity> = emptyList(),
    selectedId: Long = 0L,
    ...
    if (favouriteSongs.isEmpty()) {
        Box(modifier = modifier.fillMaxSize(), contentAlignment = Alignment.Center) {
            Text(
                text = "No favourites yet - tap the heart on any song to add it here",
